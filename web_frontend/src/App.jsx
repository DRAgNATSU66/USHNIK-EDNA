...
/* --- KEY RESPONSIVE/SCROLLABILITY TWEAKS APPLIED --- */
...
/* Allow vertical scrolling on the page so content fits on small screens.
   Preserve horizontal clipping to prevent awkward side-scroll when the UI
   intentionally uses fixed-width elements. */
html,body,#root{height:100%;width:100%;overflow-y:auto;overflow-x:hidden}
body{font-family:'Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto, 'Oxygen','Ubuntu','Cantarell',sans-serif;background:var(--gradient-bg);color:var(--text-primary);line-height:1.6;-webkit-font-smoothing:antialiased;position:relative;-webkit-text-size-adjust:100%}

/* background canvas fixed and sized to viewport */
#background-canvas{position:fixed;top:0;left:0;width:100vw;height:100vh;z-index:0;pointer-events:none}

/* Main container is scrollable so page can flow on small screens */
.main-container{position:relative;z-index:10;min-height:100vh;display:flex;flex-direction:column;overflow-y:auto;padding:2rem;-webkit-overflow-scrolling:touch}

/* header and top visuals */
...
/* content grid is responsive: 1 column by default, 2 columns at tablet, 3 columns on wide */
...
/* glass panels keep theme and are flexible in height */
...
/* results */
/* make results container scrollable — thumb styled below */
.results-container{max-height:400px;overflow-y:auto;padding-right:0.5rem;-webkit-overflow-scrolling:touch}
...
/* loading overlay (fixed on top so scrolling content doesn't break it) */
...
@keyframes glow { from { text-shadow: 0 0 5px #00ff88, 0 0 10px #00ff88, 0 0 15px #00ff88; } to { text-shadow: 0 0 20px #00ff88, 0 0 30px #00ff88, 0 0 40px #00ff88; } }

/* ============================
   CUSTOM "FLOATING" SCROLLBAR
   ============================
   - Applies to all scrollable elements in Chromium-based browsers (Chrome/Edge/Opera).
   - Track is transparent (no visible grey/white sheath).
   - Thumb is rounded, gradient electric-blue, with soft glow—matches your screenshot.
   - For Firefox we use scrollbar-color and scrollbar-width fallback.
   - We also ensure scrollbar-corner is transparent so there is no white corner box.
*/

/* Webkit / Blink (Chrome, Edge, Opera) */
*::-webkit-scrollbar {
  width: 12px;                /* thumb thickness */
  height: 12px;
}

/* Make track fully transparent so there is no sheath / visible track */
*::-webkit-scrollbar-track {
  background: transparent;
  border-radius: 999px;
}

/* The thumb — gradient electric-blue with glow and rounded pill shape */
*::-webkit-scrollbar-thumb {
  background: linear-gradient(180deg, #0066ff 0%, #00d4ff 100%);
  border-radius: 999px;
  border: 3px solid rgba(0,0,0,0); /* transparent border to avoid any visible sheath */
  box-shadow:
    0 3px 12px rgba(0,212,255,0.25),   /* outer soft glow */
    inset 0 0 6px rgba(255,255,255,0.06); /* subtle inner sheen */
}

/* Slightly larger glow on hover for feedback */
...
/* Remove corner background (where horizontal + vertical meet) */
*::-webkit-scrollbar-corner {
  background: transparent;
}

/* Firefox: thin + color fallback (thumb color + transparent track) */
html, body, .results-container, .json-preview, .list-scroll, .main-container {
  scrollbar-width: thin; /* 'auto' | 'thin' | 'none' */
  scrollbar-color: #00d4ff transparent; /* thumb track */
}

/* Also target elements that might show scrollbars in your app */
.results-container, .json-preview, .list-scroll {
  /* ensure no visible inset track or background */
  background-clip: padding-box;
}

/* Accessibility: ensure focus on scrollbar-like elements remains visible (does not add sheath) */
:focus {
  outline: none;
}

/* End scrollbar custom rules */
        `}
      </style>
